...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query1 = "SELECT COUNT(id) AS total FROM `lifemembers`"; 
    ...
    $query = "INSERT INTO lifemembers (name, mid, email, mobile, designation, affiliation, hq, photo, utr) 
    ...
    if (isset($_FILES["file"]) && $_FILES["file"]["error"] == 0) {
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $uploadFile)) {
            ...
            header("Location: ../lifemem.php?status=1");
        } else {
            echo "File upload failed.";
            header("Location: ../lifemem.php?status=2");
        }
    } else {
